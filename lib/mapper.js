  ...
  _.each(routePlan, (routeSet) => {
    _.mapKeys(routeSet.map, (value, key) => {
      ...
      if (routeSet.middleware) {
        defaultRoute.middleware = _.isArray(routeSet.middleware) ? routeSet.middleware : [routeSet.middleware]
      }
      ...
      route.pattern = util.pincanon(_.clone(route.pin)).replace(':*', `:${key}`)
      ...
      if (value.alias && !_.startsWith(value.alias, '/')) {
